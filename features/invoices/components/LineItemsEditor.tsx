...
import { invoiceTotal, lineItemTotal } from "../constants";
import { LineItemForm } from "./LineItemForm";
import type { InvoiceLineItem } from "@/generated/prisma/client";

type LineItem = InvoiceLineItem & { product: { id: string; name: string } | null };
  ...
  return (
    <div>
        ...
        <tbody>
          {lineItems.length === 0 ? (
            <TableEmptyState
              ...
              description="Add a line item to start building this invoice."
            />
          ) : (
            lineItems.map((item) => (
      ...
      {lineItems.length > 0 && (
        ...
        >
          <span>Total</span>
          <span>{formatCurrency(invoiceTotal(lineItems), currency)}</span>
        </div>
      )}
      ...
      >
        {editing === "new" ? (
          <LineItemForm
            action={addLineItemAction.bind(null, invoiceId)}
            products={products}
          ...
          <LineItemForm
            action={updateLineItemAction.bind(null, editing.id, invoiceId)}
            products={products}
            ...
            footerExtra={
              <form
                action={async () => {
                  await deleteLineItemAction(editing.id, invoiceId);
                  setEditing(null);
                }}
              >
